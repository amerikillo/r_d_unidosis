...
Connection conn__001 = DriverManager.getConnection("jdbc:mysql://localhost/r_d_unidosis", "root", "eve9397");
  ...
  Statement stmt03 = conn__001.createStatement();
	...
	if(but.equals("Guardar"))
     {
	    ...
	    rset03=stmt03.executeQuery("select unidad from receta");
		while (rset03.next()){
				unidad=rset03.getString("unidad");
				//out.print(unidad);
		}
		//out.print(unidad); 
	 ...
	 rset_001 = stmt_001.executeQuery("SELECT STR_TO_DATE('"+fechanew+"', '%d/%m/%Y')"); 
	   ...
	   stmt1.execute("insert into modificacion values ('"+clave_jv+"','"+descrip_jv+"','-','-','"+exist_jv+"','"+origen_jv+"','"+ap2_jv+"','"+total+"','"+fechanew+"','"+am22_jv+"','ENTRADA MANUAL AL INVENTARIO','"+obs_jv+"',current_timestamp,'-','-','-','"+unidad+"','-','-',0)");	   
    ...
    <td width="650"><form id="form" name="form" method="post" action="carga_clave_modi.jsp?usuario=<%=am22_jv%>">
      <table width="650" height="323" border="0" align="center" cellpadding="2">
        <tr>
          ...
          <td height="49" colspan="2" bgcolor="#FFFFFF" class="logo style1"><div align="center">Entrada por Ajuste  al Inventario <br /> PACIENTE EN CAMA</div></td>
