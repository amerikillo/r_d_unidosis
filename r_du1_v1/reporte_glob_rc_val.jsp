...
rset_001 = stmt_001.executeQuery("SELECT STR_TO_DATE('"+date2_jv+"', '%d/%m/%Y')"); 
	...
	{
 rset__001=stmt__001.executeQuery("select clave,descrip,sum(cant_sol),sum(cant_sur),present,clave_uni from receta_colectiva where unidad='"+llamo_cat_jv+"' and fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='"+org_jv+"'  and tipo_receta='rc' group by clave order by (clave+0)");
    ...
    <table width="434" border="0" align="center">
      <tr>
        <td width="418"><table width="1357" border="1">
			...
			<%
			present="1";
			stmtPendientes.execute("delete from pendientes_col where mes between '"+date2_jv+"' and '"+date2_jv+"' and origen='"+org_jv+"'");
			stmt05.execute("delete from receta_colectiva where pendientes='T' and fecha_re between '2014-"+month+"-01' and '2014-"+month+"-31' and partida='"+org_jv+"'");
    while(rset__001.next()) 
				...
				*/	
				  ...
				  rset_7=stmt_4.executeQuery("select cant from pasti_ampu where clave='"+clave+"' group by clave;");
					...
					if(present2==0)             //calculo de los pendientes por surtir.
					{
					present="1";
					present2=1;
					}
					...
					if(resto>0){
							//stmtPendientes.execute("delete from pendientes_col");
							stmtPendientes.execute("insert into pendientes_col values('"+clave+"','"+rset__001.getString("descrip")+"','"+rset__001.getString("present")+"','"+present+"','"+resto+"','"+org_jv+"','"+date2_jv+"')");
							stmtPendientes.execute("insert into receta_colectiva values('-','"+date+"','-','-','-','"+llamo_cat_jv+"','-','-','-','-','-','-','-','-','"+clave+"','"+rset__001.getString("descrip")+"','"+resto+"','"+resto+"','-','-','"+rset__001.getString("clave_uni")+"','-','-','-','-','-','-','"+org_jv+"','-','-','-','RC','-','-','-','-','-','-','-','-','-','T','-','-','-','-','-','-','-','-')");
					}	
