...
//varibles medico
String medico_jv="",uni_jv="",cedu_jv="",nomed_jv="",var_radio1="",var_radio2="";
...
try{
        prueba=request.getParameter("radio_pro");
...
if(but.equals("Modificar"))
   ...
   //if(cantinv2>0)
	 // {
			...
			stmt1.execute("insert into modificacion values ('"+clavesur+"','"+descripsur+"','-','-','"+cantinv2+"','"+partidasur+"','-"+mtotal+"','"+mtotal2+"','"+date_jv+"','"+encar_jv+"','SALIDA VÍA RECETA','',current_timestamp,'"+foliore_jv+"','"+nomed_jv+"','"+cedu_jv+"','"+nom_unidad+"','-','-',0)");
			}else{
			mtotal=cant22_jv-sur11;
			mtotal2=mtotal+cantinv2;
			stmt1.execute("insert into modificacion values ('"+clavesur+"','"+descripsur+"','-','-','"+cantinv2+"','"+partidasur+"','"+mtotal+"','"+mtotal2+"','"+date_jv+"','"+encar_jv+"','AJUSTE VÍA RECETA','',current_timestamp,'"+foliore_jv+"','"+nomed_jv+"','"+cedu_jv+"','"+nom_unidad+"','-','-',0)");
			}
	   ...
	   self.location='recedgo_fecha.jsp?radio_pro=<%=prueba%>&id_prov=<%=eliminar_jv%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=&descrip1=&present1=&indica1=<%=indica1_jv%>&sol1=&sur1=&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=encar_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&univer=<%=uni_jv%>&cedu=<%=cedu_jv%>&nomed=<%=nomed_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&part=&id_med=<%=id_med_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&carnet=<%=carnet_jv%>&programa=<%=programa_jv%>&perfil=<%=perfil%>'
	   </script>
...
<form name="form" method="post" action="modificar_new_fecha.jsp?radio_pro=<%=prueba%>&uni=<%=nom_unidad%>&juris=<%=no_jur%>&nombre=<%=nombre_jv%>&edad=<%=edad_jv%>&folio=<%=folio_jv%>&clave1=<%=clave1_jv%>&descrip1=<%=descrip1_jv%>&present1=<%=present1_jv%>&indica1=<%=indica1_jv%>&sol1=<%=sol1_jv%>&sur1=<%=sur1_jv%>&clave2=<%=clave2_jv%>&descrip2=<%=descrip2_jv%>&present2=<%=present2_jv%>&indica2=<%=indica2_jv%>&sol2=<%=sol2_jv%>&sur2=<%=sur2_jv%>&clave3=<%=clave3_jv%>&descrip3=<%=descrip3_jv%>&present3=<%=present3_jv%>&indica3=<%=indica3_jv%>&sol3=<%=sol3_jv%>&sur3=<%=sur3_jv%>&cause=<%=cause_jv%>&foliore=<%=foliore_jv%>&encar=<%=encar_jv%>&juris1=<%=juris_jv%>&clave_uni=<%=clave_jv%>&univer=<%=uni_jv%>&cedu=<%=cedu_jv%>&nomed=<%=nomed_jv%>&id_prov=<%=eliminar_jv%>&cv_dgo=<%=cv_dgo_jv%>&cv_uni=<%=cv_uni_jv%>&cv_mpio=<%=cv_mpio_jv%>&part=<%=part_jv%>&id_med=<%=id_med_jv%>&day5=<%=t1_jv%>&mes5=<%=t2_jv%>&aa5=<%=t3_jv%>&carnet=<%=carnet_jv%>&programa=<%=programa_jv%>&perfil=<%=perfil%>">

<table width="779" height="339" border="3" align="center" cellpadding="2">
  <tr>
    <td width="790" height="137"><table width="710" border="0" align="center" cellpadding="2">
      <tr>
        ...
        <td width="225"><img src="logo_salud_gob.jpg" /></td>
      </tr>
    </table>
        ...
        <tr>
          <td height="71" colspan="3" class="style4"><table width="710" border="0" align="center" cellpadding="2">
            <tr>
			  ...
			  <td width="533" class="style2"><input type="hidden" name="txtf_tipofol" size="1"  class="style4"  value="<%=prueba%>" readonly="true" /></td>
