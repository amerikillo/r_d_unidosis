...
Connection conn10 = DriverManager.getConnection("jdbc:mysql://localhost/r_d_unidosis", "root", "eve9397");
		...
		Statement stmtPendientes = conn10.createStatement();
		Statement stmt05 = conn10.createStatement();
...
String clave_unidad,llamo_cat_jv="",date1_jv="",date2_jv="",date11_jv="",date22_jv="",boton_jv="",tip_uni_jv="",date_jv="",sol="",sur="",sol_u="",sur_u="",org_jv="",dest="",precio="",present="",sur111="";
...
rset_001 = stmt_001.executeQuery("SELECT STR_TO_DATE('"+date2_jv+"', '%d/%m/%Y')"); 
	...
	{
 rset__001=stmt__001.executeQuery("select clave,descrip,sum(cant_sol),sum(cant_sur),present,clave_uni from receta where unidad='"+llamo_cat_jv+"' and fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and cant_sur not in ('0') and partida='"+org_jv+"'  and tipo_receta='rf' group by clave order by (clave+0)");
...
%>
<%
	java.util.Date fecha = new Date(month);
...
<table width="1048" border="0" align="center" cellpadding="2">
  <tr>
    ...
    PERIODO: <%=date1_jv%> al <%=date2_jv%><br /> PACIENTE CAMA</span><br />
    <br />
    </div></td>
...
<table width="20%" border="0" align="center" cellpadding="0" cellspacing="0">
 ...
 <tr>
    ...
    <table width="434" border="0" align="center">
      <tr>
        <td width="418"><table width="1357" border="1">
			...
			<%
			present="1";
			stmtPendientes.execute("delete from pendientes_far where mes between '"+date2_jv+"' and '"+date2_jv+"' and origen='"+org_jv+"'");
			stmt05.execute("delete from receta where pendientes='T' and fecha_re between '2014-"+month+"-01' and '2014-"+month+"-31' and partida='"+org_jv+"'");
    ...
    while(rset__001.next()) 
				  ...
				  prueba= rset__001.getString("present");
				...
				*/	
				  ...
				  rset_7=stmt_4.executeQuery("select cant from pasti_ampu where clave='"+clave+"' group by clave;");
					...
					//out.print(prueba);
					if(resto>0){
							//stmtPendientes.execute("delete from pendientes_far");
							stmtPendientes.execute("insert into pendientes_far values('"+clave+"','"+rset__001.getString("descrip")+"','"+rset__001.getString("present")+"','"+present+"','"+resto+"','"+org_jv+"','"+date2_jv+"')");
							stmtPendientes.execute("insert into receta values('-','"+date+"','-','-','-','"+llamo_cat_jv+"','-','-','-','-','-','-','-','-','"+clave+"','"+rset__001.getString("descrip")+"','"+resto+"','"+resto+"','-','-','"+rset__001.getString("clave_uni")+"','-','-','-','-','-','-','"+org_jv+"','"+rset__001.getString("present")+"','-','-','RF','-','-','-','-','-','-','-','-','-','T','-','-','-','-','-','-','-','0')");
					}	
	...
	%>
            <tr>
              ...
              <%
			  String clave_larga="";
				  ResultSet rset2=stmt_4.executeQuery("select clave_larga from claves_licitadas where clave='"+rset__001.getString("clave")+"' ");
			  ...
			  %>
              <td class="Estilo6"><%=clave_larga%>&nbsp;</td>
              <td class="Estilo6"><%=rset__001.getString("descrip")%>&nbsp;</td>
              
              
              <td align="center" class="Estilo9"><%=rset__001.getString("present")%></td>
