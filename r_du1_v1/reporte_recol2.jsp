...
rset_001 = stmt_001.executeQuery("SELECT STR_TO_DATE('"+date2_jv+"', '%d/%m/%Y')"); 
	...
	{
 rset__001=stmt__001.executeQuery("select servicio,clave,descrip,partida,cant_sol,cant_sur,fecha_re,folio_re,sum(cant_sur) from receta_colectiva where pendientes not in ('T') and cant_sur not in ('0') and unidad='"+llamo_cat_jv+"' and fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='"+org_jv+"' and tipo_receta='rc' group by (clave)");
	}
 ...
 <tr>
			...
			present="1";
			  while(rset__001.next()) 
				  ...
				  rset_7=stmt_4.executeQuery("select cant from pasti_ampu where clave='"+clave+"' group by clave;");
					...
					rset_1=stmt__001m.executeQuery("select servicio,clave,descrip,partida,cant_sol,cant_sur,fecha_re,folio_re,sum(cant_sur) from receta_colectiva where pendientes not in ('T') and cant_sur not in ('0') and unidad='"+llamo_cat_jv+"' and fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='"+partida_jv+"' and clave ='"+clave+"' and tipo_receta='rc'  group by (clave),servicio");
			while(rset_1.next()) 
                  {
	...
	%>
            <tr>
			  ...
			  %>
              <td><div align="center"><span class="style49"><%=clave_larga%></span></div></td>
...
rset_2=stmt__002.executeQuery("select count(folio_re) from receta_colectiva where pendientes not in ('T') and cant_sur not in ('0') and unidad='"+llamo_cat_jv+"' and fecha_re between '"+date1_jv+"' and '"+date2_jv+"' and partida='"+org_jv+"' and tipo_receta='rc' group by folio_re");
			while(rset_2.next()) 
                  {
				  folio_re2++;
